...
class ExampleApp(QtWidgets.QMainWindow, desing.Ui_MainWindow):
                ...
                if file_extension == ".jpg":
                    if name != '.DS_Store':
                        ...
                        if results[0].masks is not None:
                            mask_raw = np.zeros((512, 512, 1), dtype = np.float32)
                            boxes = []
                            for item in results[0]:
                                mask_raw = mask_raw + item.masks[0].cpu().data.numpy().transpose(1, 2, 0)
                                boxes.append(item.boxes[0].cpu().data.numpy())
                        
                            for box in boxes:
                                self.x_center = np.round((box[0][0]+box[0][2])/2)
                                self.y_center = np.round((box[0][1]+box[0][3])/2)
                                self.XY_dict[name] = [self.x_center, self.y_center]
                            mask_raw = cv2.normalize(mask_raw, None, 255, 0, cv2.NORM_MINMAX, cv2.CV_8U)
                            contours, _ = cv2.findContours(mask_raw, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                            cv2.drawContours(im, contours, -1, (0, 255, 0), 2)
                            cv2.imwrite(str(self.path_seg) + '/' + filename + '_mask.png', mask_raw) #save as mask
                            cv2.imwrite(str(self.path_seg) + '/' + filename + '_masked.png', im) #show 3rd window


                    half_width, half_height = 64 , 64
                    start_x = int(max(self.x_center - half_width, 0)) #Center is determined in get_mask, here x of the center and y of the center should be 
                    ...
                    img = frame[start_y:end_y, start_x:end_x, :] # 'this is the main image CT
                    ...
                    self.mutation_dict[name] = [self.EGFR_result, self.KRAS_result]
    ...
    def update_image(self):
        ...
        try:
            ...
            self.x_loc_label.setText(str(self.XY_dict[self.comboBox.currentText()][0]))
            self.y_loc_label.setText(str(self.XY_dict[self.comboBox.currentText()][1]))

            self.kras_label.setText(str(self.mutation_dict[self.comboBox.currentText()][1]))
            self.egfr_label.setText(str(self.mutation_dict[self.comboBox.currentText()][0]))
